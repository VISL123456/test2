...
# Initialiser session state variabler hvis de ikke finnes
if 'uploaded' not in st.session_state:
    st.session_state['uploaded'] = False
if 'feedback_submitted' not in st.session_state:
    st.session_state['feedback_submitted'] = False
if 'feedback_iso' not in st.session_state:
    st.session_state['feedback_iso'] = average_user_feedback()
if 'feedback_shutter_speed' not in st.session_state:
    st.session_state['feedback_shutter_speed'] = "1/250s"
if 'feedback_nd_filter' not in st.session_state:
    st.session_state['feedback_nd_filter'] = "Ingen"

# Last opp bildet hvis det ikke er lastet opp
...
if uploaded_file is not None:
    ...
    st.session_state['feedback_iso'] = st.number_input("Hvilken ISO brukte du?", min_value=100, max_value=6400, step=100, value=st.session_state['feedback_iso'])
    st.session_state['feedback_shutter_speed'] = st.selectbox("Hvilken lukkerhastighet brukte du?", ["1/1000s", "1/500s", "1/250s", "1/125s", "1/60s"], index=["1/1000s", "1/500s", "1/250s", "1/125s", "1/60s"].index(st.session_state['feedback_shutter_speed']))
    st.session_state['feedback_nd_filter'] = st.selectbox("Hvilket ND-filter brukte du?", ["Ingen", "ND4", "ND8", "ND16", "ND32", "ND64"], index=["Ingen", "ND4", "ND8", "ND16", "ND32", "ND64"].index(st.session_state['feedback_nd_filter']))

    if st.button("Send tilbakemelding") and not st.session_state['feedback_submitted']:
        save_feedback({"iso": st.session_state['feedback_iso'], "shutter_speed": st.session_state['feedback_shutter_speed'], "nd_filter": st.session_state['feedback_nd_filter']})
